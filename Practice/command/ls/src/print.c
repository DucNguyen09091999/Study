...
static void print_list(file_info *files, int count, options_t *options, widths_t *widths)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if (options->ll_settings.show_extra_data)
        {
            ...
            {
                if (options->ll_settings.show_readable_size)
                {
                    //print widths->size_width having length files[i].st_size
                    printf("%*ldK ", widths->size_width, files[i].st_size); // %*ld dung de can thang hang
                }
                else
                {

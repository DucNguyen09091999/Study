...
{
    widths->inode_width = max(widths->inode_width, NUM_LEN((int)file->st_ino));
    widths->nlink_width = max(widths->nlink_width, NUM_LEN((int)file->st_nlink));
    widths->ownerid_width = max(widths->ownerid_width, NUM_LEN((int)file->st_uid));
    widths->ownername_width = max(widths->ownername_width, strlen(file->owner_name));
    widths->groupid_width = max(widths->groupid_width, NUM_LEN((int)file->st_gid));
    widths->groupname_width = max(widths->groupname_width, strlen(file->group_name));

    int size_width = NUM_LEN((int)file->st_size);
    if (S_ISCHR(file->st_mode) || S_ISBLK(file->st_mode))
        size_width = NUM_LEN((int)major(file->st_rdev)) + NUM_LEN((int)minor(file->st_rdev)) + 2; // major, minor
